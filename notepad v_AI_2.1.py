...
class TextEditor:
    def __init__(self, master):
        ...
        self.text_area = Text(master, font=(self.font_name.get(), self.font_size.get()), undo=True, bg="#ffffff",
                              fg="#000000")
        ...
        # Status bar
        self.status_bar = Label(master, text="Line: 1 | Column: 0", bd=1, relief=SUNKEN, anchor=W, bg="#dcdcdc")
        ...
        Button(frame, text="Color", command=self.change_colour, bg="#007bff", fg="white").grid(row=0, column=0, padx=5)
        OptionMenu(frame, self.font_name, *font.families(), command=self.change_font).grid(row=0, column=1, padx=5)
    ...
    def find_replace(self):
        ...
        Label(find_window, text="Find:", bg="#f0f0f0").grid(row=0, column=0)
        find_entry = Entry(find_window)
        find_entry.grid(row=0, column=1)

        Label(find_window, text="Replace with:", bg="#f0f0f0").grid(row=1, column=0)
    ...
    def replace_all(self, find_text, replace_text):
        ...
        new_content = re.sub(
            find_text.replace("\\", "\\\\").replace(".", "\\.").replace("*", "\\*").replace("?", "\\?"), replace_text,
            content)  # Escape regex special characters
        self.text_area.delete(1.0, END)
